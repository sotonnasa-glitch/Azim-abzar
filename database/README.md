...
- چند migration جدید برای hardening در `supabase/migrations/` نسخه‌بندی شده‌اند؛ در محیط‌های جدید ترجیحاً migrationها را اجرا کنید، نه اینکه فقط فایل‌های legacy این پوشه را مستقل اجرا کنید.
- آخرین hardeningها شامل محافظت ربات تلگرام، کنترل نقش‌های admin/editor، snapshot اطلاعات سفارش، اعتبارسنجی checkout، حذف direct-write به تاریخچه تخفیف و جلوگیری از over-refund هستند.
- در محیط موجود، کاتالوگ فعال ۹۰۸ محصول است؛ اسکریپت‌های قدیمی نباید آن را به تعداد دیگری seed کنند.
- تاریخچه migrationهای قدیمی production به‌طور کامل در repository بازیابی نشده است؛ migrationهای جدیدی که در این کار ایجاد شده‌اند قابل بازسازی و اجرای مجدد هستند.
